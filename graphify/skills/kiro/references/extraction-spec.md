...
Scratch file RULE: every file you write other than CHUNK_PATH must carry your chunk number in its name (`scratchpad/gen_CHUNK_NUM.py`, not `scratchpad/gen.py`). Agents run concurrently on one filesystem; two that pick the same helper path overwrite each other, and the loser then runs the other's script and writes the wrong chunk numbers. Both writes succeed, so nothing errors.
```
