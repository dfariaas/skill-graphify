...
Before Pass 3, converters turn supported structured, pointer, and binary formats
into Markdown sidecars under `graphify-out/converted/`. CSV and TSV conversion is
built in, bounded, and content-versioned. Office files (`.docx`, `.xlsx`) use the
`[office]` extra. Google Workspace shortcuts (`.gdoc`, `.gsheet`, `.gslides`) are
opt-in with `--google-workspace` or `GRAPHIFY_GOOGLE_WORKSPACE=1` and require an
