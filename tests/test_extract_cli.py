...
def _postgres_result():
    """Minimal valid database extraction returned by the introspection stub."""
    source = "postgresql:/db.example.test/app"
    return {
        "nodes": [
            {
                "id": "postgresql:app:public.widgets",
                "label": "public.widgets",
                "type": "table",
                "file_type": "code",
                "source_file": source,
                "source_location": f"{source}:1",
...
@pytest.mark.parametrize("with_path", [False, True], ids=["postgres-only", "path-and-postgres"])
def test_extract_postgres_reaches_introspection_and_writes_output(
    monkeypatch, tmp_path, with_path
):
    """PostgreSQL-only mode must initialize the empty detection result.

    The no-path form previously skipped ``detect()`` as intended, then crashed
    with ``UnboundLocalError`` when common reporting code read ``detection``.
    The path form is covered alongside it to preserve combined source + schema
    extraction while fixing the database-only branch.
    """
    out_dir = tmp_path / "out"
    argv = ["graphify", "extract"]
    if with_path:
        corpus = tmp_path / "corpus"
        corpus.mkdir()
        (corpus / "app.py").write_text("def application_entry():\n    return 1\n")
    ...
    assert calls == ["postgresql://reader:secret@db.example.test/app"]

    graph_path = out_dir / "graphify-out" / "graph.json"
    graph = json.loads(graph_path.read_text(encoding="utf-8"))
    labels = {node.get("label") for node in graph["nodes"]}
    assert "public.widgets" in labels
    if with_path:
        assert any(str(label).startswith("application_entry") for label in labels)
